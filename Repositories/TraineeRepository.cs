...
namespace TraineeManagement.Api.Repositories
{
    public class TraineeRepository : ITraineeRepository
    {
        ...
        public async Task<(List<Trainee> trainees, int totalRecords)> GetTraineesAsync(SearchDTO<TraineeStatusType> searchDTO)
        {
            ...
            var trainees = await Query.Skip((searchDTO.PageNumber - 1) * searchDTO.PageSize).Take(searchDTO.PageSize).ToListAsync();
        ...
        {
            return await _context.Trainees.AnyAsync() ? await _context.Trainees.MaxAsync(t => t.Id) + 1 : 1;
        }
